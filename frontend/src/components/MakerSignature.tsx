...
/**
 * Author credit for the footer.
 *
 * Everything personal lives in the object below, so updating a handle or adding
 * a platform is a one-line change and never touches the markup.
 *
 * Kept deliberately separate from the hisab disclaimer it sits beside: that
 * line is a substantive caveat about what the app's output means, this is a
 * byline. Merging them would dilute the disclaimer, which is the more important
 * of the two.
 */
const MAKER = {
  ...
  links: [
    {
      ...
      // The only stroked icon here - it is lucide, matching the rest of the
      // app's iconography. The three brand marks below are solid because that
      // is the form each platform publishes.
      icon: <Globe className="size-[18px]" strokeWidth={1.75} aria-hidden="true" />,
    },
    {
      label: "GitHub",
...
export function MakerSignature() {
  // Evaluated when this server component renders, which for the static export
  // is build time - the year is baked into the published HTML.
